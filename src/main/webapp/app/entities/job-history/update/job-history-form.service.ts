 ...
 */
type FormValueOf<T extends IJobHistory | NewJobHistory> = Omit<T, 'startDate' | 'endDate' | 'date'> & {
...
type JobHistoryFormDefaults = Pick<NewJobHistory, 'id' | 'startDate' | 'endDate' | 'date'>;
...
export class JobHistoryFormService {
  ...
  private convertJobHistoryRawValueToJobHistory(
    ...
    return {
      ...
      date: dayjs(rawJobHistory.date, DATE_TIME_FORMAT),
    };
  }
  ...
  private convertJobHistoryToJobHistoryRawValue(
    ...
    return {
      ...
      date: jobHistory.date ? jobHistory.date.format(DATE_TIME_FORMAT) : undefined,
    };
  }
}
