...
describe('Location e2e test', () => {
  ...
  describe('new Location page', () => {
    ...
    it('should create an instance of Location', () => {
      cy.get(`[data-cy="streetAddress"]`).type('jusque premièrement');
      cy.get(`[data-cy="streetAddress"]`).should('have.value', 'jusque premièrement');

      cy.get(`[data-cy="postalCode"]`).type('comme');
      cy.get(`[data-cy="postalCode"]`).should('have.value', 'comme');

      cy.get(`[data-cy="city"]`).type('Neuilly-sur-Seine');
      cy.get(`[data-cy="city"]`).should('have.value', 'Neuilly-sur-Seine');

      cy.get(`[data-cy="stateProvince"]`).type('sans que croâ');
      cy.get(`[data-cy="stateProvince"]`).should('have.value', 'sans que croâ');
