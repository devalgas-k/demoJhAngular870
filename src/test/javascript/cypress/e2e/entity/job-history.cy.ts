...
describe('JobHistory e2e test', () => {
  ...
  describe('new JobHistory page', () => {
    ...
    it('should create an instance of JobHistory', () => {
      cy.get(`[data-cy="startDate"]`).type('2024-09-02T00:24');
      cy.get(`[data-cy="startDate"]`).blur();
      cy.get(`[data-cy="startDate"]`).should('have.value', '2024-09-02T00:24');

      cy.get(`[data-cy="endDate"]`).type('2024-09-01T09:01');
      cy.get(`[data-cy="endDate"]`).blur();
      cy.get(`[data-cy="endDate"]`).should('have.value', '2024-09-01T09:01');

      cy.get(`[data-cy="language"]`).select('SPANISH');

      cy.setFieldImageAsBytesOfEntity('file', 'integration-test.png', 'image/png');

      cy.get(`[data-cy="date"]`).type('2024-09-02T04:22');
      cy.get(`[data-cy="date"]`).blur();
      cy.get(`[data-cy="date"]`).should('have.value', '2024-09-02T04:22');

      cy.get(`[data-cy="duration"]`).type('PT2M');
      cy.get(`[data-cy="duration"]`).blur();
      cy.get(`[data-cy="duration"]`).should('have.value', 'PT2M');
