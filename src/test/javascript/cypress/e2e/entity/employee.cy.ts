...
describe('Employee e2e test', () => {
  ...
  const employeeSample = { email: 'CH3qg@9A`O4.*8x' };
  ...
  describe('new Employee page', () => {
    ...
    it('should create an instance of Employee', () => {
      cy.get(`[data-cy="firstName"]`).type('Eusébie');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Eusébie');

      cy.get(`[data-cy="lastName"]`).type('Paul');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Paul');

      cy.get(`[data-cy="email"]`).type('q$N;@ER^.LO');
      cy.get(`[data-cy="email"]`).should('have.value', 'q$N;@ER^.LO');

      cy.get(`[data-cy="phoneNumber"]`).type('horrible étant donné que de par');
      cy.get(`[data-cy="phoneNumber"]`).should('have.value', 'horrible étant donné que de par');

      cy.get(`[data-cy="hireDate"]`).type('2024-09-01T06:47');
      cy.get(`[data-cy="hireDate"]`).blur();
      cy.get(`[data-cy="hireDate"]`).should('have.value', '2024-09-01T06:47');

      cy.get(`[data-cy="salary"]`).type('31877');
      cy.get(`[data-cy="salary"]`).should('have.value', '31877');

      cy.get(`[data-cy="commissionPct"]`).type('20185');
      cy.get(`[data-cy="commissionPct"]`).should('have.value', '20185');

      cy.get(`[data-cy="level"]`).type('8');
      cy.get(`[data-cy="level"]`).should('have.value', '8');
